...
public class Iteration2SortByTitleAscendingTest {
    ...
    public void sortByTitleAscendingTest() {
        ...
        onView(withId(R.id.z_a_sort)).perform(click());
        onView(withId(R.id.a_z_sort)).check(matches(not(isChecked())));
        onView(withId(R.id.z_a_sort)).check(matches(isChecked()));

        onView(withId(R.id.save_changes_button)).perform(click());

        onView(withId(R.id.my_archive_settings)).check(matches(isDisplayed()));

        assert (MyArchiveSettings.getSortSetting() == MyArchiveSettings.Sort.TITLE);
        assert (MyArchiveSettings.getOrderSetting() == MyArchiveSettings.SortOrder.ASCENDING);
    }
}
