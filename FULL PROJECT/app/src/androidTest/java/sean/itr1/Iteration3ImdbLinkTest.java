...
public class Iteration3ImdbLinkTest {
    @Rule
    public ActivityTestRule<SearchPage> AddSeriesActvityTestRule = new ActivityTestRule<>(SearchPage.class);

    // Issue: Wikipedia and Imbd links: Scenario 2
    @Test
    public void ImbdLinkTests() {
        ...
        Intents.init();
        Matcher<Intent> expectedIntent = allOf(hasAction(Intent.ACTION_VIEW), hasData("http://www.imdb.com/title/tt0102926/"));
        intending(expectedIntent).respondWith(new Instrumentation.ActivityResult(0, null));
        onView(withId(R.id.imdb)).perform(click());
